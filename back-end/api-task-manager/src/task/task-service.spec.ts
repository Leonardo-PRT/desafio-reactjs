...
describe('TaskService', () => {
  ...
  describe('getTaskById', () => {
    it('should return a task by id', async () => {
      ...
      expect(prisma.task.findUnique).toHaveBeenCalledWith({
        where: { id: 1 },
        include: {
          TaskTag: { include: { tag: { select: { id: true, title: true } } } },
        },
      });
    });
  ...
  describe('updateTask', () => {
    it('should update a task', async () => {
      ...
      (prisma.taskTag.findUnique as jest.Mock).mockResolvedValue([
        { taskId: 1, tagId: 1 },
      ]);
    ...
    it('should throw NotFoundException if task is not found', async () => {
      ...
      await expect(service.update(taskId, updateTaskDto, 1)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
